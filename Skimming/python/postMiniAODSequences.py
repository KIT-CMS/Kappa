...
def create_postMiniAODSequences(year,dataset_type):
    ...
    process.jercVars = cms.EDProducer("BetaStarPackedCandidateVarProducer",
        srcJet = cms.InputTag("selectedUpdatedPatJetsUpdatedJEC"),    
        srcPF = cms.InputTag("packedPFCandidates"),
        maxDR = cms.double(0.4)
    )
    from RecoJets.JetProducers.QGTagger_cfi import  QGTagger
    process.qgtagger=QGTagger.clone(srcJets="selectedUpdatedPatJetsUpdatedJEC",srcVertexCollection="offlineSlimmedPrimaryVertices")
    ...
    process.selectedUpdatedPatJetsUpdatedJECUpdatedbReg = cms.EDProducer("PATJetUserDataEmbedder",
        src = cms.InputTag("updatedJetsWithUserData"),
        userFloats = cms.PSet(
    ...
    process.bjetRegSequence = cms.Sequence(process.bJetVars*process.jercVars*process.qgtagger*process.updatedJetsWithUserData*process.bjetNN*process.selectedUpdatedPatJetsUpdatedJECUpdatedbReg)
    process.p *= process.bjetRegSequence
