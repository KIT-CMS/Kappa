...
def get_sample_by_nick(nickname, expect_n_results = 1):
	...
	query = {
		...
		"format": "^" + split_nick[4] + "$",
                ...
                "extension": "^" + split_nick[-1].split('-')[0] + "$" if 'ext' in split_nick[-1] else None,
                "version": "^" + version + "$" if split_nick[4] != 'MINIAOD' else None,
	}
	if 'Embedding' in nickname:
		query.update({'generator': ''})
		if len(split_nick) == 5 and "Embedding2016" in nickname:
			query["version"] = None
        if len(split_nick) > 6:
