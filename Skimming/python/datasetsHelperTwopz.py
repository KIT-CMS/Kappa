...
class datasetsHelperTwopz:
    ...
    def make_nickname(self, sample_dict):
        ...
        version_part = ""
        if sample_dict["extension"] != "" and sample_dict["version"] != "":
            version_part = "-" + sample_dict["version"]
        elif sample_dict["version"] != "":
            version_part = "_" + sample_dict["version"]
        nick += version_part
        return nick
    ...
    def isData(self, nick):
        ...
        if "data" not in self.base_dict[nick].keys():
            raise Exception('Sample with nick: ' + nick + ' is missing the data key. ')
